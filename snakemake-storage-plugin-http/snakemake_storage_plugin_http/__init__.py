...
class HTTPTokenAuth(AuthBase):
    ...
    def __call__(self, r):
        r.headers["Authorization"] = "Bearer " + self.auth_service.access_token
        return r
...
def parse_auth(arg):
    ...
    elif auth_type == "HTTPTokenAuth":
        return HTTPTokenAuth(matches.group("arg"))
    else:
        raise WorkflowError(
            f"Authentication type {auth_type} not supported. "
            f"Please choose one of HTTPBasicAuth, HTTPDigestAuth, OAuth1 or HTTPTokenAuth"
        )
