...
class StorageObject(StorageObjectRead, StorageObjectWrite, StorageObjectGlob):
    ...
    def exists(self) -> bool:
        ...
        try:
            response = self.s3bucket().meta.client.list_objects(
                Bucket=self.bucket, Delimiter="/", Prefix=self.key
            )
            ...
            if "Contents" in response:
                for obj in response["Contents"]:
                    key = (
                        obj["Key"].split("/", 1)[-1]
                        if obj["Key"].startswith(self.bucket)
                        else obj["Key"]
                    )
                    if key == self.key:
                        return True
            ...
            if "CommonPrefixes" in response:
                for obj in response["CommonPrefixes"]:
                    prefix = (
                        obj["Prefix"].split("/", 1)[-1]
                        if obj["Prefix"].startswith(self.bucket)
                        else obj["Prefix"]
                    )
    ...
    def mtime(self) -> float:
        ...
        key = self.key + "/" if self.is_dir() else self.key

        response = self.s3bucket().meta.client.list_objects(
            Bucket=self.bucket, Delimiter="/", Prefix=key
        )
        ...
        if self.is_dir():
            return max(
                item["LastModified"].timestamp() for item in response["Contents"]
            )
        else:
            return response["Contents"][0]["LastModified"].timestamp()
    ...
    def size(self) -> int:
        ...
        resposne = self.s3bucket().meta.client.list_objects(
            Bucket=self.bucket, Delimiter="/", Prefix=self.key
        )

        if "Contents" not in resposne:
            return 0

        return resposne["Contents"][0]["Size"]
    ...
    def get_subkeys(self):
        ...
        return (
            item
            for item in self.s3bucket().objects.filter(Delimiter="/", Prefix=prefix)
            if item.key != prefix
        )
    ...
    def bucket_exists(self):
        try:
            self.s3bucket().meta.client.list_objects(
                Bucket=self.bucket, Delimiter="/", MaxKeys=1
            )
        except botocore.exceptions.ClientError as e:
